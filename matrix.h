...
public:
	...
	//makes a 1 x vec.size() matrix
	Matrix(const Vec& orig);
	...
	/// Returns the inverse of this matrix. In cases where the matrix is not invertible, returns the least-squares approximation to its inverse.
	Matrix* pseudoInverse();

protected:
	void singularValueDecompositionHelper(Matrix** ppU, double** ppDiag, Matrix** ppV, bool throwIfNoConverge, size_t maxIters);
	void fixNans();
};
